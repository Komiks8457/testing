...
inline wstring LowerString(const wstring &text)
{
    ...
    unsigned int iLength = static_cast<unsigned int>(text.size() + 1);
...
inline wstring URLEncoding(const wstring &keyword, bool convertToUTF8 = true)
{
    ...
    if (convertToUTF8)
    {
        iLength = static_cast<int>(::WideCharToMultiByte(CP_UTF8,
                                        0,
                                        keyword.c_str(),
...
inline unsigned int GetSeparateKeywordMatchGrade(const wstring &source, const wstring &keyword)
{
    ...
    else
    {
        ...
        {
            unsigned int compareLength = static_cast<unsigned int>(lowerKeyword.length() - index);
        ...
        grade = static_cast<unsigned int>(matchLength * 100 / lowerKeyword.length());
    }
...
inline unsigned int GetKeywordMatchGrade(const wstring &source, const wstring & keyword)
{
    ...
    if (SeparateString(keyword, L" ", results) && results.size() > 0)
    {
        ...
        {
            keywordTotalLength += static_cast<unsigned int>(results[index].length());
        }
        for (vector<wstring>::size_type index = 0; index < results.size(); index++)
        {
            grade += static_cast<unsigned int>(GetSeparateKeywordMatchGrade(src, results[index]) * results[index].length() / keywordTotalLength);
        }
    }
    else
    {
...
inline wstring MD5(const wstring &text)
{
	...
	if (::CryptAcquireContext(&hCryptProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT | CRYPT_MACHINE_KEYSET))
	{
		...
		{
			if (::CryptHashData(hHash, (BYTE *)asciiText.c_str(), static_cast<DWORD>(asciiText.size()), 0))
			{
				BYTE result[16];
				DWORD dwSize = 16;
				wchar_t temp[3] = L"";
...
inline wstring GetHost(const wstring &url)
{
    ...
    unsigned int index = static_cast<unsigned int>(urlWidthoutHttp.find(L"/"));
    if (index == wstring::npos)
    {
        index = static_cast<unsigned int>(urlWidthoutHttp.find(L"\\"));
    }
    if (index == wstring ::npos)
    {
